...
- **書籍一覧の参照**  
  - 社内の書籍一覧、貸出情報、貸出者履歴、社員の書籍への口コミを閲覧可能
  - 楽天bookAPIで取得した書籍の中から購入リクエストができて、それらは管理者へ通知される。購入リクエストをする際にuserが所持しているpoint(1pt=1円)を消費する。月毎にchargeされ1ヶ月経つと消えるmonthly_pointと,口コミ投稿または社員からのプレゼントで得られるspecial_pointが存在。
- **評価と口コミ**  
  - 社内図書に対し、5段階評価と口コミを投稿可能。その投稿は社内タイムラインに表示される。口コミを投稿すると、月毎に消えないspecial_pointを獲得できる。
  - 口コミには「いいね」機能を備え、他の社員の口コミに多様なスタンプでリアクション可能
- **プレゼント機能**  
  - 口コミ投稿で得られたspecial_pointは社員へ贈ることが可能。
- **読書記録**
  - マイページにて自分の読書ログと残point含むuser情報を閲覧可能
  - 他ユーザーの読書ログを閲覧可能
  - 読書ログはバーチャル本棚としての閲覧も可能
  - 日々の読書内容、ページ数、学んだことを記録していける。その記録は社内タイムラインに表示される(非公開可能)
...
- **書籍リクエスト管理とポイント設定**  
  ...
  - ポイント管理として、社員ごとの所有ポイントの設定・閲覧・編集が可能
- **書籍と貸出情報の管理**
